...
export class WikiContainer {
  ...
  private evees = new EveesModule(
    [this.ethEvees, this.httpEvees],
    this.httpEvees
  );
  ...
  async initializeMicroOrchestrator(web3provider, dispatcher, hasHomeProposal) {
    ...
    try {
      ...
      customElements.define(
        'simple-wiki',
        SimpleWiki(web3provider, dispatcher, hasHomeProposal)
      );
    } catch (e) {
      console.log(e);
    }
  ...
  public static getInstance(web3provider, dispatcher, hasHomeProposal) {
    return (
      this._instance ||
      (this._instance = new this(web3provider, dispatcher, hasHomeProposal))
    );
  }
}
